...
function extractValue(arr, key) {
    return arr.reduce(function(extract, obj) {
        extract.push(obj[key])
        return extract
    }, [])
}
...
function vowelCount(str) {
    const vowels = "aeiou"
    return str.toLowerCase().split('').reduce(function(extract, char) {
...
function addKeyAndValue(arr, key, value) {
    return arr.reduce(function(accumulator, currentObj) {
        accumulator.push({ ...currentObj, [key]: value })
        return accumulator
    }, [])
}
...
function partition(arr, callback) {
    return arr.reduce(function(extract, value){
        if (callback(value)){
            extract[0].push(value)
        } else {
            extract[1].push(value)
        }
        return extract
    },[[],[]])
}
